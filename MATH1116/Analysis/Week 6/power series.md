...
# Motivation 
Recall that 
$$\ln(1+x)=\int_{0}^x \frac{1}{1+t}dt=\int_{0}^x \sum_{n=0} ^\infty(-t)^n dt$$
...
Thus, $$\int_{0}^x \sum_{n=0}^\infty (-t)^ndt=\sum_{n=0}^\infty \int_{0}^x(-t)^ndt=\sum_{n=0}^\infty (-1)^n \frac{x^{n+1}}{n+1}$$
...
Let $0<\bar{\rho}<\rho<r\quad \forall n\in \mathbb{N}$ $$\left| na_{n} \bar{\rho} ^{n-1}\right|\leq \frac{1}{\bar{\rho}}|a_{n}\rho^n|{n} \cdot \left( \frac{\bar{\rho}}{\rho} \right) ^n $$
...
$(2)$
Let $N  \in \mathbb{N}$, $x \in(c,c+r)$. We have that $$\sum_{n=0}^N a_{n}(x-c)^n = \int_{c}^x\sum_{n=1}^N na_{n}(t-c)^{n-1}dt \xrightarrow[N \to \infty]{}\int_{c}^x \sum_{n=0}^\infty na_{n}(t-c)^{n-1}dt$$via [[uniform convergence and L1 norm convergence of sequence of functions|uniform convergence]]. 
