...
$\theta_{ML}=\frac{1}{n}\sum_{i=1}^N x_{i}$ - a very simple answer, which is also known as just $\frac{x_{1}+\dots+x_{n}}{n}$. 
...
Recall that $$\underbrace{p(\theta \mid X)}_{\text{posterior}} = \frac{\overbrace{p(X \mid \theta)}^{\text{likelihood}}\ \overbrace{p(\theta)}^{\text{prior}}}{\underbrace{p(X)}_{\text{evidence}}}$$
