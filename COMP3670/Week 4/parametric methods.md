...
# Definition 
A parametric, statistical model is a family of distributions $\{ p(\cdot |\theta)\theta \in \Theta \subset \mathbb{R}^m\}$ indexed by finitely many numbers $\theta=(\theta_{1},\dots,\theta_{m})$ To assume the model is to assume that the unkown distribution is $p(\cdot | \theta^*)$ for some unknown $\theta^* \in \Theta$. 
