...
For binary classification with binary [[loss function]],

$$\mathbb{P}(max_{h \in \mathcal{H}} \left|   E_{out}(h)-E_{in}(h) \right| >\epsilon) \leq8(N+1)^v e^{-N\epsilon^2/32}$$
Basically, there is some guarantee that we are able to shrink the [[generalisation gap]].

S, $E_{out}(h)\leq E_{in}(h)+\sqrt{ \frac{32}{N}\left[ v\log(N+1)+\log \frac{8}{\delta} \right] }$
With the term on the right being our complexity penalty.

### Properties:
Fixed $v$, the penalty $\xrightarrow[N \to \infty]{}0$
Fixed $N$, penalty grows with $v$. 

If $v=\infty$ , there is no bound at all. 
...
> [!theorem]
> For [[Empirical risk minimisation]] with the binary loss, if $d_{\text{VC}}(\mathcal{H}) = v < \infty$,
>
> $$
