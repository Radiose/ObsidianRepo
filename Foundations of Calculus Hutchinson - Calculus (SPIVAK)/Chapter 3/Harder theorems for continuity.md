...
#### Theorem 1
 If $f,\ g$ are continuous at $a$, then 
...
#### Theorem 3: 
Suppose $x$ is continuous at $a$ and $f(a) > 0$. Then, there is a number $\delta >0$ such that $f(x)>0$ for all $x$ satisfying $|x-a|< \delta$. 
Similarly, if $f(a)<0$, then there is a number $\delta > 0$ such that $f(x)<0$ for all $x$ satisfying $|x-a|<\delta$

Proof 
Consider the case $f(a)>0$. Since $f$ is continuous at $a$, if $\epsilon > 0$, then there is a $\delta>0$ such that 
$\forall x \in \mathcal{D(f)} \ \ |x-a|<\delta \implies|f(x)-f(a)|<\epsilon$.
Since $f(a)>0$, we can substitute $\epsilon = f(a)$
$|x-a|<\delta \implies |f(x)-f(a)|<f(a)$
The RHS implies that $f(x)>0 \blacksquare$
A similar proof can be utilised for the other case $f(a)<0$, where $\epsilon = -f(a)$
